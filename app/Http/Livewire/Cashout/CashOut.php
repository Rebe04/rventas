...
class CashOut extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.cashout.cash-out', compact('users'))
    ...
    public function viewDetails(Sale $sale)
    {
        ...
        $this->details = Sale::join('sale_details as d', 'd.sale_id', 'sale_id')
            ->join('products as p', 'p.id', 'd.product_id')
            ->select('d.sale_id', 'p.name as product', 'd.quantity', 'd.price')
            ->whereBetween('sales.created_at', [$fi, $ff])
            ->where('sales.status', 'Paid')->where('sales.user_id', $this->userid)
